...
public class AnalyzerConsult {
	 ...
	 *  || is the logical operator "Or"
	 */
	static final String patternOr = "((^[\\w\\%\\' ]+)\\|\\|)" +									  
	  								 		"(([\\w\\%\\' ]+)\\|\\|)*" +
	  								 		"([\\w\\%\\' ]+$)";
	...
	public String makeWhere(Base obj){
		...
		for (String pPropertyField[]: rf.execute(obj)){						
			...
			if (pPropertyField[1].trim().equals("n") && (pPropertyField[2].trim().equals("0") || pPropertyField[2].trim().equals("0.0"))) {
		...
		if (whereClause.startsWith(" and") || whereClause.startsWith(" or")) whereClause = whereClause.substring(4).trim();
	...
	/**
	 * Make the sentence
	 * @param obj  The properties to field (name, logical operator and value) 
	...
	private String analyzer(String pPropertyField[]){
		...
		if (mat.matches()){
			...
			String endResult =  pPropertyField[0] + " " +  operator + typeStringOpen + mat.group(5).trim()+  typeStringClose +")";
			...
			while ( mat.find() ) {
				...
				result = result + pPropertyField[0] + " " +  operator + typeStringOpen + mat.group(2).trim() + typeStringClose +" or ";
